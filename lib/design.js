...
// "entities".replace(/s$/,'') is "entitie". One table, so a generated task never
// says a word nobody would write.
const ONE = {
  modules: 'area', entities: 'business object', serverUnits: 'unit', serverFunctions: 'function',
  apiRoutes: 'endpoint', frontendUnits: 'screen', events: 'event', processes: 'journey',
  statusFlows: 'lifecycle', reactions: 'reaction',
};
const oneOf = (dim) => ONE[dim] || String(dim || '').replace(/s$/, '');
...
/**
 * A field id, in the shape the model uses.
 *
 * Fields are the level a change actually lands on — "the order carries a discount"
 * is a field, and a design that can only declare whole objects cannot say it.
 */
export function fieldId(entityId, name) {
  return `f-${safe(String(entityId).replace(/^ent-/, ''))}-${safe(name)}`;
}
...
export function takenIds(model, items) {
  ...
  for (const it of items) {
    t.add(it.id);
    for (const f of (it.object && it.object.fields) || []) if (f && f.id) t.add(f.id);
  }
  return t;
}

/** Look a field up in the model or in what has only been declared. */
export function findField(id, model, items) {
  for (const e of (model.entities || [])) for (const f of (e.fields || [])) if (f.id === id) return { field: f, on: e, declared: false };
  for (const it of items) if (it.dim === 'entities') {
    for (const f of (it.object.fields || [])) if (f.id === id) return { field: f, on: it.object, declared: true };
  }
  return null;
}
...
export function conformance(items, model) {
  ...
  for (const it of items) {
    ...
    // Declared fields are checked like declared links: an object that exists
    // without the field somebody asked for is built, and not what was drawn.
    for (const f of (it.object.fields || [])) {
      if (!f || !f.id) continue;
      const held = !!(real && (real.fields || []).some((x) => x && x.id === f.id));
      links.push({ kind: 'field', label: 'has a field', to: f.id, held, fieldName: f.name });
    }
 ...
 */
/**
 * One task for a whole slice.
 *
 * A journal drawn as six elements is one piece of work, and six tasks for it is
 * six times the ceremony and one shared thing nobody owns. The checks are the
 * union: every element and every relationship the slice declared has to be in the
 * rebuilt model before it is done.
 */
export function taskForSlice(items, model, title) {
  const c = conformance(items, model);
  const rows = c.rows.filter((r) => r.state !== 'present');
  if (!rows.length) return null;
  const touches = new Set();
  const verify = [];
  for (const r of rows) {
    touches.add(r.id);
    // An element that is already there does not need a check that it is there; the
    // only thing outstanding is the part of it that was declared and is missing.
    if (r.state === 'missing') {
      verify.push(`\`${r.id}\` (${r.name}) exists in .gitmir/model after a rebuild`);
    }
    for (const l of r.links) {
      if (l.to) touches.add(l.to);
      if (l.held) continue;
      verify.push(l.kind === 'field'
        ? `the model records that ${r.name} has a field ${l.fieldName || l.to}`
        : `the model records that ${r.name} ${l.label} ${labelOf(l.to, model) || l.to}`);
    }
  }
  verify.push('the Design view reports every element of this slice as present');
  const body = [
    `${rows.length} element(s) declared on the product map, built as one piece of work.`,
    '',
    ...rows.map((r) => {
      const bits = r.links.map((l) => l.kind === 'field'
        ? `has a field \`${l.fieldName || l.to}\``
        : `${l.label} \`${l.to}\``);
      return `- **${r.name}** (${oneOf(r.dim)})${bits.length ? ' — ' + bits.join(', ') : ''}`;
    }),
  ].join('\n');
  return { title: title || `Build ${rows.length} declared elements`, body, touches: [...touches], verify };
}
  ...
  for (const r of c.rows) {
    ...
    const verify = [
      r.state === 'missing'
        ...
        : `\`${r.id}\` is still in .gitmir/model after a rebuild`,
      ...r.links.filter((l) => !l.held).map((l) => l.kind === 'field'
        ? `the model records that ${r.name} has a field ${l.fieldName || l.to}`
    ...
    out.push({
      ...
      id: r.id,
      body: [
        it && it.note ? it.note : `Declared on the product map as a ${oneOf(r.dim)}.`,
        '',
