...
export function reworkTree(rows, names = {}) {
  const byId = new Map(), byArea = new Map();
  const put = (map, key, r) => {
  ...
  for (const r of rows) {
    for (const id of (r.objects || [])) put(byId, id, r);
    for (const a of (r.areas || [])) put(byArea, a, r);
  }
  const shape = (map) => [...map.entries()].map(([id, v]) => {
  ...
  return { objects: shape(byId), areas: shape(byArea) };
}
